...
def onboard(cluster_name, namespace, api_key, secret):
    ...
    create_namespace = os.system (f'oc create namespace {namespace}')
    result_cgd9 = os.system(f'oc apply -f {cluster_name}.{cluster_id}.yaml')
    result_d9_assets = os.system(f'oc create -f cp-cloudguard-openshift.yaml --namespace {namespace}')
...
def remove(k8s_yaml, api_key, secret):
    ...
    *cluster_name, cluster_id, _ = k8s_yaml.split('.')
    ...
    result_cgd9 = os.system(f'oc delete -f {k8s_yaml}')
    result_d9_assets = os.system(f'oc delete -f cp-cloudguard-openshift.yaml --namespace {namespace}')
